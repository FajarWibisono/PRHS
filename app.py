...
if st.session_state.data is not None:
    ...
    # Pre-initialize BVPS session state key agar metric header bisa membaca nilai terbaru
    _bvps_sym = st.session_state.get("symbol", "")
    _bvps_param_key = f"bvps_param_{_bvps_sym}"
    if _bvps_param_key not in st.session_state:
        _bvps_init = data["current_bvps"]
        st.session_state[_bvps_param_key] = float(round(_bvps_init, 2)) if _bvps_init is not None else 0.0

    # Company header
    sub = f" · {data['sector']} / {data['industry']}" if data.get("sector") else ""
    st.subheader(f"📊 {data['company_name']}{sub}")

    # Key metrics — BVPS & PBV menggunakan nilai dari Parameter Estimasi (bisa diedit user)
    m1, m2, m3, m4, m5 = st.columns(5)
    cp  = data["current_price"]
    eps = data["current_eps"]
    bv  = data["current_bvps"]
    bv_display = st.session_state.get(_bvps_param_key) or bv
    ...
    m3.metric("BVPS",            fmt_rp(bv_display)      if bv_display else "N/A")
    m4.metric("PER Saat Ini",    f"{cp/eps:.2f}x"        if (cp is not None and eps is not None and eps > 0) else "N/A")
    m5.metric("PBV Saat Ini",    f"{cp/bv_display:.2f}x" if (cp is not None and bv_display and bv_display > 0) else "N/A")
    ...
    with col_left:
        ...
        bvps_input = st.number_input(
            ...
            step=10.0,
            format="%.2f",
            help="Book Value Per Share dari laporan keuangan terakhir. Mengubah nilai ini juga memperbarui BVPS & PBV di ringkasan atas.",
            key=_bvps_param_key,
        )
